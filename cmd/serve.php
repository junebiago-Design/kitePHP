...
if (is_resource($process)) {
    while (!feof($pipes[1])) {
        ...
        } elseif (preg_match('/^\s*ROUTE:/i', $line)) {
            // Cyan output for matched routes, e.g.
            // ROUTE: GET /contact => [ContactController::class, 'index']
            echo "\033[36m[ROUTE] " . trim(preg_replace('/^\s*ROUTE:\s*/i', '', $line)) . "\033[0m\n";
        } elseif (preg_match('/^\s*FILE:/i', $line)) {
            // Magenta output for files loaded/included, e.g.
            // FILE: /public/assets/css/style.css
            echo "\033[35m[FILE] " . trim(preg_replace('/^\s*FILE:\s*/i', '', $line)) . "\033[0m\n";
